#ifndef XADMASTER_SEVENZIP_C
#define XADMASTER_SEVENZIP_C
...
// Start of auto-generated collection of 7-Zip C code
...
// End of auto-generated collection of 7-Zip C code
...
XADCLIENTVERSTR("7-Zip 1.0 (19.06.2006)")

#define SEVENZIP_VERSION           1
#define SEVENZIP_REVISION          0

#define SEVENZIP_BUFSIZE 16384

#define SZPAI(a) ((struct sevenzip_archive_private *) ((a)->xai_PrivateClient))
#define SZPFI(a) ((struct sevenzip_file_private *) ((a)->xfi_PrivateInfo))


struct sevenzip_stream
{
	ISzInStream instream;
	...
	xadERROR err;
	xadUINT8 buf[SEVENZIP_BUFSIZE];
};

struct sevenzip_archive_private
{
	struct sevenzip_stream stream;
	ISzAlloc alloc;
	ISzAlloc alloctemp;
	CArchiveDatabaseEx db;
...
SZ_RESULT sevenzip_read(void *object,void **buffer,size_t maxsize,size_t *actualsize)
{
	...
	if(maxsize)
	if(s->err=xadHookAccess(s->xmb,XADAC_READ,maxsize,s->buf,s->ai)) return SZE_FAIL;

	*buffer=s->buf;
	if(actualsize) *actualsize=maxsize;
	return SZ_OK;
}
...
XADRECOGDATA(SevenZip)
{
	// should scan for SFX?
	return TestSignatureCandidate(data);
}
...
XADGETINFO(SevenZip)
{
	...
	ai->xai_PrivateClient=xadAllocVec(XADM sizeof(struct sevenzip_archive_private),XADMEMF_CLEAR);
	if(!ai->xai_PrivateClient) return XADERR_NOMEMORY;

	ai->xai_Flags=XADAIF_FILEARCHIVE;

	SZPAI(ai)->stream.instream.Read=sevenzip_read;
	SZPAI(ai)->stream.instream.Seek=sevenzip_seek;
	SZPAI(ai)->stream.ai=ai;
	SZPAI(ai)->stream.xmb=xadMasterBase;
	SZPAI(ai)->stream.err=XADERR_OK;

	SZPAI(ai)->alloc.Alloc=SzAlloc;
	SZPAI(ai)->alloc.Free=SzFree;

	SZPAI(ai)->alloctemp.Alloc=SzAllocTemp;
	SZPAI(ai)->alloctemp.Free=SzFreeTemp;

	SZPAI(ai)->blockindex=0xFFFFFFFF;
	SZPAI(ai)->outbuf=NULL;
	SZPAI(ai)->outsize=0;

	InitCrcTable();

	SzArDbExInit(&SZPAI(ai)->db);

	SZ_RESULT res=SzArchiveOpen(&SZPAI(ai)->stream.instream,&SZPAI(ai)->db,&SZPAI(ai)->alloc,&SZPAI(ai)->alloctemp);
	if(res==SZ_OK)
	{
		for(int i=0;i<SZPAI(ai)->db.Database.NumFiles;i++)
		{
			CFileItem *f=SZPAI(ai)->db.Database.Files+i;

			struct xadFileInfo *fi=(struct xadFileInfo *)xadAllocObject(XADM XADOBJ_FILEINFO,
				XAD_OBJPRIVINFOSIZE,sizeof(struct sevenzip_file_private),
				XAD_OBJNAMESIZE,strlen(f->Name),
			TAG_DONE);
			if(!fi)  return XADERR_NOMEMORY;

			strcpy(fi->xfi_FileName,f->Name);
			fi->xfi_Flags=XADFIF_EXTRACTONBUILD|XADFIF_NODATE;
			if(f->IsDirectory) fi->xfi_Flags|=XADFIF_DIRECTORY;
			fi->xfi_Size=f->Size;
			fi->xfi_CrunchSize=0;
			SZPFI(fi)->crc=f->FileCRC;
			SZPFI(fi)->num=i;
	...
	{
		if(SZPAI(ai)->stream.err) return SZPAI(ai)->stream.err;
		else return XADERR_DATAFORMAT;
	}

	return XADERR_OK;
}

XADUNARCHIVE(SevenZip)
{
	...
	SZ_RESULT res=SzExtract(&SZPAI(ai)->stream.instream,&SZPAI(ai)->db,SZPFI(fi)->num,
	&SZPAI(ai)->blockindex,&SZPAI(ai)->outbuf,&SZPAI(ai)->outsize,&offset,&size,
	&SZPAI(ai)->alloc,&SZPAI(ai)->alloctemp);
	if(res==SZ_OK)
	{
		if(err=xadHookTagAccess(XADM XADAC_WRITE,size,SZPAI(ai)->outbuf+offset,ai,
			XAD_GETCRC32,&crc,
		TAG_DONE)) return err;

		if(~crc!=SZPFI(fi)->crc) return XADERR_CHECKSUM;
	}
	else return XADERR_DATAFORMAT;
...
XADFREE(SevenZip)
{
	SZPAI(ai)->alloc.Free(SZPAI(ai)->outbuf);
	SzArDbExFree(&SZPAI(ai)->db,SZPAI(ai)->alloc.Free);
	xadFreeObjectA(XADM ai->xai_PrivateClient,NULL);
}



XADFIRSTCLIENT(SevenZip)
{
	XADNEXTCLIENT,
	XADCLIENT_VERSION,
	XADMASTERVERSION,
	...
	"7-Zip",
	/* client functions */
	XADRECOGDATAP(SevenZip),
	XADGETINFOP(SevenZip),
	XADUNARCHIVEP(SevenZip),
	XADFREEP(SevenZip),
};

#undef XADNEXTCLIENT
#define XADNEXTCLIENT XADNEXTCLIENTNAME(SevenZip)

#endif /* XADMASTER_SEVENZIP_C */
